            ...
            ->each(function (array $attributes, $index) use ($import) {
                $this->toModels($import, $attributes, $index)->each(function (Model $model) use ($import) {
                    try {
                        if ($import instanceof WithUpserts) {
                            $dataUnique = [];
                            if (is_string($import->uniqueBy())) {
                                $dataUnique[$import->uniqueBy()] = $model->getAttributes()[$import->uniqueBy()];
                            } else {
                                foreach ($import->uniqueBy() as $unique) {
                                    $dataUnique[$unique] = $model->getAttributes()[$unique];
                                }
                            }
                            $model->updateOrCreate($dataUnique,$model->getAttributes());
                        } else {
                            $model->saveOrFail();
                        }
                    } catch (Throwable $e) {
